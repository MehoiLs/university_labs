...
class MatrixDraft {
    ...
    void normalizeMainAndSubDiagonal() {
        for (auto i = 0; i < size; i++) {
            ...
            b[i] /= coef; //TODO: ЗДЕСЬ НЕ ДЕЛИТЬ НА КОЭФ, А СРАЗУ СЭТАТЬ "1"
            q[i] /= coef;
            f[i] /= coef;
            ...
            if (i < size - 1) {
                ...
                a[i] -= coef2 * b[i];  //TODO: ЗДЕСЬ НЕ ВЫЧИТАТЬ КОЭФ, А СРАЗУ СЭТАТЬ "0"
                b[i+1] -= coef2 * c[i];
                f[i+1] -= coef2 * f[i];
    ...
    static MatrixDraft *fromFile(const std::string &filename, const bool forceOverrideValues = false) {
        ...
        return new MatrixDraft(n, k, a, b, c, p, q, f);
    }
